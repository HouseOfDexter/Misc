...
type
  ...
  public
    ...
    class procedure IsNotAssigned(Value : TObject;
                                  const ValueDescr : String = '';
                                  const MethodName : String = ''); overload;
    class procedure IsAssigned(Value : IUnknown;
                                  const ValueDescr : String = '';
                                  const MethodName : String = ''); overload;

    class procedure IsAssigned(Value : Pointer;
                               const ValueDescr : String = '';
                               const MethodName : String = ''); overload;

    class procedure IsAssigned(Value : TMethod;
                               const ValueDescr : String = '';
                               const MethodName : String = ''); overload;
    ...
    class procedure FileExists(const Filename : String;
                               ...
                               const MethodName : String = '');
    class procedure DirExists(const Directory : String;
                               const DirectoryDescr : String = '';
                               const MethodName : String = '');
...
class procedure TmtCheck.IsAssigned(Value: IInterface; const ValueDescr,
  MethodName: String);
begin
  if not Assigned(Value) then
    RaiseError(ValueDescr, 'Must be Assigned', MethodName);
...
class procedure TmtCheck.IsAssigned(Value: Pointer; const ValueDescr,
  MethodName: String);
begin
  if not Assigned(Value) then
    RaiseError(ValueDescr, 'Must be Assigned', MethodName);
...
class procedure TmtCheck.IsAssigned(Value: TMethod; const ValueDescr,
  MethodName: String);
begin
  if (not Assigned(Value.Code)) and (not Assigned(Value.Data) ) then
    RaiseError(ValueDescr, 'Must be Assigned', MethodName);
end;
...
class procedure TmtCheck.FileExists(const Filename, FilenameDescr,
  MethodName: String);
begin
  IsNotEmpty(Filename, 'Filename', 'TmtCheck.FileExists');
  if not SysUtils.FileExists(Filename) then
    RaiseError(FilenameDescr, 'File must exist', MethodName);
end;

class procedure TmtCheck.DirExists(const Directory, DirectoryDescr,
  MethodName: String);
begin
  IsNotEmpty(Directory, 'Directory', 'TmtCheck.DirExists');
  if not SysUtils.DirectoryExists(Directory) then
    RaiseError(DirectoryDescr, 'Directory must exist', MethodName);
end;
